...
echo "barcode,$prefix" >$prefix.barcode_counts.txt
...
do
    ...
    r1_count=`zcat $R1 | grep -c $barcode`
    r2_count=`zcat $R2 | grep -c $barcode`
    total=$(($r1_count + $r2_count))
    echo "$barcode,$total" >>$prefix.barcode_counts.txt
done
